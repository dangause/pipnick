...
def astrometry_all(maindir, api_key, use_table=False, resolve=False,
                   excl_files=[], excl_objs=[], excl_filts=[], source_list=None):
    """
    Run astrometry.net calibration on all images in the specified directory.
    
    If no source_list is provided (default is None), this function uses 
    ast.solve_from_image() to obtain the astrometric solution.
    
    When a source_list (a list of tuples) is provided, each tuple (for the
    corresponding image) is unpacked into x and y coordinate arrays and passed
    to ast.solve_from_source_list().
    ...
    use_table : bool, optional
        ...
        to automatically exclude files that have been commented-out.
    ...
    source_list : list of tuple, optional
        Optional list of tuples for source coordinates. Each tuple should contain
        two elements (x, y) which are passed to ast.solve_from_source_list() for the
        corresponding image.
    ...
    def check_wcs_header(wcs_header, output_path):
        if wcs_header:
            ...
            logger.warning("Solution failed; skipping this image")
    
    for i, image_path in enumerate(mod_paths):
        ...
        # TODO: fix this when photometry is fixed
        try:
            if source_list is None:
                # Original call using the image file
                ...
                # Use the provided source list for the astrometric solution
                with fits.open(image_path) as ccd:
                    # Unpack the tuple (x, y) for this image.
                    x, y = source_list[i]
                    wcs_header = ast.solve_from_source_list(
                        x=x,
                        y=y,
                        image_width=ccd[0].header['naxis1'],
                        image_height=ccd[0].header['naxis2'],
                        radius=fov_size_approx/60,  # Max error of RA/Dec guess, in deg
                        scale_units='arcminwidth',
                        scale_type='ul',
                        scale_lower=fov_size_approx*0.8,
            ...
            logger.info("No solution found on first try; will revisit after all images are submitted")
    ...
    for submission_id in unsolved_submission_ids:
        try:
            ...
            logger.warning("No solution found on second try; skipping.")
